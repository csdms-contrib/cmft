...
integer, parameter:: cell=5000,cellS=floor(cell*dx/dxC),tmax=100000000! bottom cell size, water cell cell size, max # iter
...
call gnuplotExecute("set terminal x11")
write(str, *) "set xrange [",dx,":",cell*dx,"]", char(0) 
call gnuplotExecute(str)
write(str, *) "set yrange [",-4,":",2,"]", char(0)
call gnuplotExecute(str)
call gnuplotExecute("set ytic -5,0.5,11")
write(str, *) "set ylabel 'elevation [m]'", char(0)
call gnuplotExecute(str)
write(str, *) "set xlabel 'transect distance [m]'", char(0)
call gnuplotExecute(str)
...
open (unit=1,file="h.dat",status="replace")
i=1
do while (zg(i)>h)
i=i+1
end do
...
write (str, *) "set title 't=",tempo/365,"years", char(0)  
call gnuplotExecute(str)
write (str, *) "plot 'zg.dat' w p ti 'zg','zgin.dat' w l ti 'zg_in','h.dat' w l ti 'h'"
call gnuplotExecute(str) 
call gnuplotFlush()
